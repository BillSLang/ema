	...
	,methods:{
		initForm(){
		  ...
		  this.tableIns = table.render({
			...
			table.on('toolbar(tasteTable)', function(obj){
				  switch(obj.event){
				  	case 'export':
				  		vm.exportData();
				  	break;
			...
			var data = table.checkStatus('tastelist').data;
	    	if(data.length !== 1){
		...
		selectOnceMore(){
			...
			layer.open({
				type:2,
				title:'添加品牌',
				content:baseURL+'/taste/tasteEdit.html/'+table.checkStatus('tastelist').data[0].id,
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
			...
			layer.open({
				type:2,
				title:'添加品牌',
				content:baseURL+'/taste/tasteEdit.html',
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
			...
			layer.open({
				...
				async yes(index,layero){
					...
					await $.post(baseURL+'/taste/delete',param)
