	...
	,methods:{
		initForm(){
		  ...
		  this.tableIns = table.render({
				...
				cols:  [[ // 标题栏
					...
					,{field: 'site' , title:'网站',align:'center'}
			...
			table.on('toolbar(uriTable)', function(obj){
				  switch(obj.event){
				  	case 'export':
				  		vm.exportData();
				  	break;
			...
			var data = table.checkStatus('urilist').data;
	    	if(data.length !== 1){
		...
		selectOnceMore(){
			...
			layer.open({
				type:2,
				title:'添加品牌',
				content:baseURL+'/uri/uriEdit.html/'+table.checkStatus('urilist').data[0].id,
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
			...
			layer.open({
				type:2,
				title:'添加品牌',
				content:baseURL+'/uri/uriEdit.html',
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
			...
			layer.open({
				...
				async yes(index,layero){
					...
					await $.post(baseURL+'/uri/delete',param)
