var vm = new Vue({
	...
	,methods:{
		initForm(){
			form.render(null,'productCodeForm');
		},exportData(){
		  ...
		  this.tableIns = table.render({
			...
			table.on('toolbar(productCodeTable)', function(obj){
				  switch(obj.event){
				  	case 'export':
				  		vm.exportData();
				  	break;
			...
			var data = table.checkStatus('productCodelist').data;
	    	if(data.length !== 1){
		...
		selectOnceMore(){
			...
			layer.open({
				type:2,
				title:'添加产品码',
				content:baseURL+'/productCode/productCodeEdit.html/'+table.checkStatus('productCodelist').data[0].id,
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
			...
			layer.open({
				type:2,
				title:'添加产品码',
				content:baseURL+'/productCode/productCodeEdit.html',
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
			...
			layer.open({
				...
				async yes(index,layero){
					...
					await $.post(baseURL+'/productCode/delete',param)
