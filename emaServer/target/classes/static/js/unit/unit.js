	...
	,methods:{
		initForm(){
		  ...
		  this.tableIns = table.render({
			...
			var data = table.checkStatus('unitlist').data;
	    	if(data.length !== 1){
		...
		selectOnceMore(){
			...
			layer.open({
				type:2,
				title:'添加品牌',
				content:baseURL+'/unit/unitEdit.html/'+table.checkStatus('unitlist').data[0].id,
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
			...
			layer.open({
				type:2,
				title:'添加品牌',
				content:baseURL+'/unit/unitEdit.html',
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
			...
			layer.open({
				...
				async yes(index,layero){
					...
					await $.post(baseURL+'/unit/delete',param)
