	...
	,methods:{
		initForm(){
		  ...
		  this.tableIns = table.render({
			...
			table.on('toolbar(foodTypeTable)', function(obj){
				  switch(obj.event){
				  	case 'export':
				  		vm.exportData();
				  	break;
			...
			var data = table.checkStatus('foodTypelist').data;
	    	if(data.length !== 1){
			...
			layer.open({
				type:2,
				title:'添加类型',
				content:baseURL+'/foodType/foodTypeEdit.html/'+table.checkStatus('foodTypelist').data[0].id,
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
		...
		selectOnceMore(){
			...
			layer.open({
				type:2,
				title:'添加类型',
				content:baseURL+'/foodType/foodTypeEdit.html',
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
			...
			layer.open({
				...
				async yes(index,layero){
			    	...
			    	console.log(param)
					await $.post(baseURL+'/foodType/delete',param)
