	...
	,methods:{
		initForm(){
		  ...
		  this.tableIns = table.render({
				...
				cols:  [[ // 标题栏
					...
					,{field: 'domaine' , title:'域名',align:'center'}
			...
			var data = table.checkStatus('sitelist').data;
	    	if(data.length !== 1){
		...
		selectOnceMore(){
			...
			layer.open({
				type:2,
				title:'添加品牌',
				content:baseURL+'/site/siteEdit.html/'+table.checkStatus('sitelist').data[0].id,
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
			...
			layer.open({
				type:2,
				title:'添加品牌',
				content:baseURL+'/site/siteEdit.html',
				area:['300px','400px'],
				end(){
					vm.tableIns.reload();
				}
			...
			layer.open({
				...
				async yes(index,layero){
					...
					await $.post(baseURL+'/site/delete',param)
